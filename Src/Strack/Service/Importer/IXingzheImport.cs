    ...
    ) : IXingzheImport
{
    public async Task ImportAsync(long userId, long workoutId)
    {
        ...
        var detail = await api.GetWorkoutDetailAsync(workoutId);
        var points = await api.GetWorkoutStreamAsync(workoutId);
    ...
    protected async Task ImportCyclingAsync(long userId, WorkoutDetail activity, List<Record> samplings)
    {
