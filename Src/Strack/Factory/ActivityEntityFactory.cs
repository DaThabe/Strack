...
public static class ActivityEntityFactory
{
    ...
    public static ActivityEntity FromWorkoutDetail(WorkoutDetail workoutDetail, IEnumerable<XingZhe.Model.User.Workout.Record.Record> records)
    {
        ...
        SourceEntity sourceEntity = new() { Activity = activityEntity, ActivityId = activityEntity.Id, Type = SourceType.XingZhe };

        sourceEntity.XingZhe = new XingZheData() { Source = sourceEntity, SourceId = sourceEntity.Id, WorkoutId = workoutDetail.Id, UserId = workoutDetail.User.Id };
    ...
    public static ActivityEntity FromActivityDetail(ActivityDetail activityDetail, IEnumerable<Common.Model.File.Fit.Record> records)
    {
        //活动实体
        ActivityEntity activityEntity = new() { Source = null! };
        SourceEntity sourceEntity = new() { Activity = activityEntity, ActivityId = activityEntity.Id, Type = SourceType.IGSport };

        sourceEntity.IGPSport = new IGPSportData() { Source = sourceEntity, SourceId = sourceEntity.Id, ActivityId = activityDetail.Id, UserId = activityDetail.UserId };
        activityEntity.Source = sourceEntity;
    ...
    //行者记录点
    private static void CopyToEntity(XingZhe.Model.User.Workout.Record.Record model, RecordEntity entity)
    {
    ...
    //Fit记录点
    private static void CopyToEntity(Common.Model.File.Fit.Record model, RecordEntity entity)
    {
        entity.UnixTimeSeconds = model.Timestamp?.ToUnixTimeSeconds();
        entity.Longitude = model.Longitude;
        entity.Latitude = model.Latitude;
