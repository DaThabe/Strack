...
namespace Strack.Service.Importer;

public interface IXingzheImportService
{
    /// <summary>
    /// 导入
    /// </summary>
...
internal class XingzheImportService(
    ILogger<XingzheImportService> logger,
     IXingzheApi xingzheApi,
     StrackDbContext dbContext
    ) : IXingzheImportService
{
    public async Task ImportAsync(long userId, long workoutId)
    {
        logger.LogTrace("正在同步行者活动, Uid:{uid}, WorkoutId:{wid}", userId, workoutId);

        if(await dbContext.XingzheSourceData.Where(x => x.UserId == userId && x.ActivityId == workoutId).AnyAsync())
        {
            throw new InvalidOperationException("活动已存在, 无法同步");
        }

        var detail = await xingzheApi.GetActivityDetailAsync(userId, workoutId);
        var points = await xingzheApi.GetTrackPointDetailAsync(userId, workoutId);


        switch(detail.Sport)
        {
            case XingzheActivityType.Ride:
    ...
    protected async Task ImportCyclingAsync(long userId, ActivityDetail activity, List<SamplingDetail> samplings)
    {
        ...
        //活动时间
        activityEntity.MovingTime = samplings.Select(x => x.Timestamp.UtcDateTime).CalcTotalSpan();
        //暂停时间
        activityEntity.PauseTime = activityEntity.TotalTime - activityEntity.MovingTime;


        //温度
        (activityEntity.MinTemperatureCelsius, activityEntity.MaxTemperatureCelsius, activityEntity.AvgTemperatureCelsius, _) = samplings.Select(x => x.Temperature.DegreesCelsius).CalcMinMaxAvgSum();

        //爬升
        (activityEntity.TotalAscentMeters, activityEntity.TotalDescentMeters) = samplings.Select(x => x.Altitude.Meters).CalcCumulativeGainLoss();
        ...
        //功率
        (_, activityDataEntity.MaxPowerWatts, activityDataEntity.AvgPowerWatts, activityDataEntity.TotalPowerWatts) = samplings.Select(x => x.Power.Watts).CalcMinMaxAvgSum();



        void CreateEntity(out ActivityEntity activityEntity, out CyclingActivityDataEntity activityDataEntity)
        {
            var sourceDataEntity = new XingzheSourceDataEntity()
            {
                Source = null!,
                SourceId = Guid.Empty,
